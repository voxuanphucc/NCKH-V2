...
export function TreeMediaGallery({
  ...
  useEffect(() => {
    const fetchMedia = async () => {
      try {
        setLoading(true);
        const res = await mediaService.getTreeMedia(treeId);
    ...
    if (mediaItem.mediaFileType === 'IMAGE') {
      return (
        <img
          ...
          alt={mediaItem.description || mediaItem.fileName || 'Media'}
              ...
              {filteredMedia.map(mediaItem => (
                <div
                  ...
                  <div className="absolute top-2 right-2 bg-black bg-opacity-50 text-white p-1 rounded">
                    {getMediaIcon(mediaItem.mediaFileType)}
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="w-80 flex flex-col bg-warm-50 rounded-lg p-4 overflow-hidden">
            ...
            <div className="space-y-3 text-sm">
              <div>
                ...
                <p className="text-warm-900">{selectedMedia.mediaFileType}</p>
              </div>
              ...
              <div>
                <p className="text-warm-600 font-medium">Tên tệp</p>
                <p className="text-warm-900 break-all">{selectedMedia.fileName}</p>
              </div>
